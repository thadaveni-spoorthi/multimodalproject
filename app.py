...
# Make sure to replace YOUR_API_KEY with your actual Gemini API key string
client = genai.Client(
...
with gr.Blocks() as demo:
    ...
    # Wire up the buttons
    btn_extract.click(fn=extract_text, inputs=[file], outputs=[output, audio])
    btn_summarize.click(fn=summarize, inputs=[file], outputs=[output, audio])
    btn_audio.click(fn=audio_summary, inputs=[file], outputs=[output, audio])
    btn_qa.click(fn=answer_question, inputs=[file, question], outputs=[output, audio])
